...
export default function ClientePagamentosAtrasados() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        setLoading(true);
        setError("");
        const data = await apiRequest("/cliente/pagamentos/atrasados");
        const normalized = normalizeListResponse(data);
        if (!cancelled) setItems(normalized.items);
      } catch (err) {
        if (!cancelled) setError(err?.message || "Erro ao carregar pagamentos atrasados");
      } finally {
    ...
    <DashboardLayout>
      ...
      <MDBox pt={6} pb={3}>
        <Grid container spacing={6}>
          <Grid item xs={12}>
            <Card>
              <GradientHeader title="Pagamentos atrasados" color="error" />
              <MDBox pt={3}>
                {error ? (
                  <MDBox px={3} pb={2}>
