...
export default function ClienteApolicesAtivas() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        setLoading(true);
        setError("");
        const data = await apiRequest("/cliente/apolices/ativas");
        const normalized = normalizeListResponse(data);
        if (!cancelled) setItems(normalized.items);
      } catch (err) {
        if (!cancelled) setError(err?.message || "Erro ao carregar apólices ativas");
      } finally {
    ...
    const rows = items.map((apolice) => ({
      ...
      action: (
        <MDTypography
          ...
          color="info"
          fontWeight="medium"
          onClick={() => navigate(`/cliente/apolices/${apolice?.id_apolice}`)}
    ...
    <DashboardLayout>
      ...
      <MDBox pt={6} pb={3}>
        <Grid container spacing={6}>
          <Grid item xs={12}>
            <Card>
              <GradientHeader title="Apólices ativas" color="success" />
              <MDBox pt={3}>
                {error ? (
                  <MDBox px={3} pb={2}>
